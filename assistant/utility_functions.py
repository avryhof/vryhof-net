...
def search_model(input_value, **kwargs):
    ...
    if isinstance(input_value, (str, unicode)) and model and field:
        ...
        simple_text = (
            re.sub("[^A-Za-z0-9]", "", input_value).strip().replace(" ", "").lower()
        )
        ...
        for item in model_items:
            ...
            simple_field_text = (
                re.sub("[^A-Za-z0-9]", "", field_value).strip().replace(" ", "").lower()
            )
...
def intent_response(text, app_name):
    ...
    resp = {
        "body": {
            "version": API_V1,
            "response": {
                "outputSpeech": {"type": "SSML", "ssml": ssml.speak()},
                "card": {"type": "Simple", "title": app_name, "content": text},
